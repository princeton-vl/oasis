...
class InstanceParameterLoss(nn.Module):
    ...
    def forward(self, segmentation, sample_segmentation, sample_params, valid_region, gt_depth, return_loss=True):
        ...
        valid_inferred_depth = inferred_depth.view(1, -1)[:, valid_region] # (1, number of pixels)
        ...
        isinf = torch.isinf(abs_distance).cpu().detach().numpy().item() 
        if isinf:
            # import pdb; pdb.set_trace()
            # pass
            # abs_distance.fill(2.0)
            valid_inferred_depth.masked_fill_(torch.isinf(valid_inferred_depth), 2.0)
            abs_distance = torch.mean(torch.abs(valid_inferred_depth - valid_depth))
