...
def eval(_run, _log):
    ...
    os.makedirs(cfg.output_path + '/seg', exist_ok=True)
    os.makedirs(cfg.output_path + '/mask', exist_ok=True)
    ...
    with torch.no_grad():
        ...
        for line in tqdm(lines):
            ...
            f = open(os.path.join(seg_path, image_name.split('.')[0] + '.txt'), 'w')
            for j in range(1, num_ins + 1):
                m = (predict_segmentation == j).astype(np.float)
                m = m * 255.0
                m = m.astype(np.uint8)
                m = cv2.resize(m, (ow, oh), interpolation=cv2.INTER_NEAREST)
                cv2.imwrite(os.path.join(seg_path, image_name.split('.')[0] + '_{}.png'.format(j)), m)
                f.write(image_name.split('.')[0] + '_{}.png'.format(j) + ' 1 1.0\n')
            f.close()
