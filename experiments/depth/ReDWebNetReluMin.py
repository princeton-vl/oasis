...
class ReDWebNetReluMin(nn.Module):
    def __init__(self, scale_factor = 1.0):
        super(ReDWebNetReluMin, self).__init__()
       
        

        print("===================================================")
        print("Using ReDWebNetReluMin, min is 250.0 * %g" % (scale_factor))   
        print("\tLoading pretrained resnet 50...")
        print("==================================================="        )

        self.min_focal = 250.0 * scale_factor
        ...
        self.fc_focal = nn.Linear(2048,1)
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self,x):
        ...
        #branch 2:        
        temp = self.avgpool(grey12_12_2048)
        temp = temp.view(-1, 2048)


        log_disp = self.fc(temp)        # [batch_size, 1]   
        disp = torch.exp(log_disp)      # take the exponential
        disp = disp.unsqueeze(-1).unsqueeze(-1)
        disp = disp.expand(-1, -1, out.shape[2], out.shape[3])
        

        log_focal = self.fc_focal(temp) # [batch_size, 1]
        focal = torch.exp(log_focal) + self.min_focal    # take the exponential, add a minimum   

        out = out + disp            # [batch_size, 1, H, W]
        out = self.relu(out) + 1e-8
...
class ReDWebNetReluMin_raw(nn.Module):
    def __init__(self, scale_factor = 1.0):
        super(ReDWebNetReluMin_raw, self).__init__()
       
        

        print("===================================================")
        print("Using ReDWebNetReluMin Raw, min is 250.0 * %g" % (scale_factor))   
