...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, b_fc=True):
        self.inplanes = 64
...
def resnet50(pretrained=False, b_fc = True, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(block=Bottleneck, layers=[3, 4, 6, 3], b_fc=b_fc, **kwargs)
    if pretrained:            
        if b_fc:
            model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
        else:
            model.load_state_dict(torch.load('resnet50_no_fc.bin'))
            

    return model


def resnet101(pretrained=False, b_fc = True, **kwargs):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], b_fc=b_fc, **kwargs)
    if pretrained:
        if b_fc:
            model.load_state_dict(model_zoo.load_url(model_urls['resnet101']))
        else:
            model.load_state_dict(torch.load('resnet101_no_fc.bin'))
    return model
